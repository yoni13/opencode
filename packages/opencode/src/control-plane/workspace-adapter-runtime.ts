...
const context = (sourceDirectory?: string) =>
  Effect.gen(function* () {
    return {
...
export const configure = (adapter: WorkspaceAdapter, info: WorkspaceInfo, sourceDirectory?: string) =>
  Effect.gen(function* () {
    const ctx = yield* context(sourceDirectory)
